...
do_validate({_U, '*'}) -> ok;
do_validate({year, Y}) when erlang:is_integer(Y), Y >= 0 -> ok;
do_validate({year, _}) -> {error, year};
do_validate({month, M}) when erlang:is_integer(M), M >= 1, M =< 12 -> ok;
...
do_validate({day, D}) when erlang:is_integer(D), D >= 1, D =< 31 -> ok;
do_validate({day, _}) -> {error, day};
do_validate({hour, H}) when erlang:is_integer(H), H >= 0, H =< 23 -> ok;
do_validate({hour, _}) -> {error, hour};
do_validate({minute, M}) when erlang:is_integer(M), M >= 0, M =< 59 -> ok;
...
next(day, '*', 31) -> [1, 31];
next(day, '*', D) -> [1, D, D + 1];
next(day, S, _) when is_atom(S) -> lists:seq(1, 31);
next(day, S, _) -> [S];
next(hour, '*', 23) -> [0, 23];
next(hour, '*', H) -> [0, H, H + 1];
next(hour, H, _) -> [H];
