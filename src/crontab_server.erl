...
%% Types -----------------------------------------------------------
-record(s, {
    tasks = gb_trees:empty(), %% name -> task
...
-record(task, {spec, mfa, next, options}).
...
handle_call({remove, {Name, Options}}, _From, S) ->
    case do_remove(Name, Options, S#s.tasks, S#s.queue, S#s.p2n, S#s.n2p) of
...
handle_info(tick, S) ->
    {Tasks, Queue, P2N, N2P} = do_tick(S#s.tasks, S#s.queue, S#s.p2n, S#s.n2p),
    {noreply, S#s{tasks = Tasks, queue = Queue, p2n = P2N, n2p = N2P}};
...
handle_info({'EXIT', Pid, Rsn}, S) ->
    ...
    {noreply, S#s{p2n = dict:erase(Pid, S#s.p2n), n2p = dict:erase(Name, S#s.n2p)}};
...
do_remove(Name, Options, Tasks, Queue, P2N0, N2P0) ->
    case gb_trees:lookup(Name, Tasks) of
        {value, #task{next = Time, options = TaskOptions}} ->
            ...
            {ok, {gb_trees:delete(Name, Tasks), gb_trees:delete_any({Time, Name}, Queue), P2N, N2P}};
        none ->
            {error, no_such_task}
    end.
...
do_tick(Tasks0, Queue0, P2N0, N2P0) ->
    case gb_trees:size(Queue0) of
        ...
        _ ->
            Now = crontab_time:now(),
            case gb_trees:take_smallest(Queue0) of
                {{Time, Name}, Name, Queue1} when Time =< Now ->
...
try_start(Name, Task, P2N, N2P) ->
    ...
    case dict:is_key(Name, N2P) of
        ...
        false ->
            ...
            {dict:store(Pid, Name, P2N), dict:store(Name, Pid, N2P)}
    end.
